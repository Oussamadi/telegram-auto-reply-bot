from telegram import Update, ReplyKeyboardMarkup
from telegram.ext import ApplicationBuilder, MessageHandler, CommandHandler, ContextTypes, filters

# توكن البوت
TOKEN = "8242893940:AAGQzM2HfFtJkpdO2R5hI_J7Ao1ins41AzM"

# Telegram ID الخاص بك (لتلقي الرسائل)
ADMIN_ID = 1764395818  # ضع رقمك الحقيقي هنا

# ===== رسالة البداية + الأزرار =====
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        ["👤 من أنت؟", "💰 الأسعار"],
        ["🕒 أوقات العمل", "📞 تواصل معنا"],
        ["❓ مساعدة", "👨‍💼 التحدث مع شخص"]
    ]
    reply_markup = ReplyKeyboardMarkup(
        keyboard=keyboard,
        resize_keyboard=True,
        one_time_keyboard=False
    )

    await update.message.reply_text(
        "أهلاً وسهلاً 👋😊\n"
        "أنا المساعد الآلي الخاص بك 🤖\n"
        "جاهز نجاوبك ونعاونك في أي وقت.\n\n"
        "اختر من الأزرار أو اكتب سؤالك مباشرة 👇",
        reply_markup=reply_markup
    )

# ===== الردود الذكية + إشعار للمشرف =====
async def auto_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.lower()
    user = update.message.from_user

    # إشعار للمشرف (لك)
    notify_text = (
        "📩 رسالة جديدة للبوت\n\n"
        f"👤 الاسم: {user.first_name}\n"
        f"🔗 المستخدم: @{user.username}\n"
        f"🆔 ID: {user.id}\n\n"
        f"💬 الرسالة:\n{text}"
    )
    await context.bot.send_message(
        chat_id=ADMIN_ID,
        text=notify_text
    )

    # الردود الطبيعية
    if any(word in text for word in ["السلام", "السلام عليكم", "سلام"]):
        reply = "وعليكم السلام ورحمة الله وبركاته 🌸\nكيف نقدر نعاونك اليوم؟ 😊"

    elif any(word in text for word in ["مرحبا", "أهلا", "اهلا", "hello", "hi"]):
        reply = "مرحباً بيك 👋🙂 نورتنا!"

    elif "كيف حالك" in text:
        reply = "الحمد لله بخير 😊\nإن شاء الله تكون بخير أنت أيضًا 🌷"

    elif "من أنت" in text:
        reply = (
            "أنا بوت رد تلقائي 🤖\n"
            "مهمتي نرد عليك بسرعة ونقدّم لك المعلومات الأساسية.\n"
            "ولو حاب تتكلم مع شخص حقيقي، اختر الزر 👨‍💼"
        )

    elif "الأسعار" in text:
        reply = (
            "💰 بخصوص الأسعار:\n"
            "الأسعار تختلف حسب نوع الخدمة أو الطلب.\n"
            "ابعث لنا التفاصيل وسنرد عليك بدقة 👍"
        )

    elif "أوقات" in text:
        reply = (
            "🕒 أوقات العمل:\n"
            "من السبت إلى الخميس\n"
            "من 09:00 صباحًا إلى 17:00 مساءً ⏰"
        )

    elif "تواصل" in text or "اتصال" in text:
        reply = (
            "📞 طرق التواصل:\n"
            "📱 واتس اب 0669272484\n"
            "✉️ تلغرام: من هنا 😊"
        )

    elif "مساعدة" in text:
        reply = (
            "❓ يسعدني نعاونك\n"
            "تقدر تسأل عن:\n"
            "- الأسعار\n"
            "- أوقات العمل\n"
            "- الخدمات\n"
            "- أي استفسار آخر\n\n"
            "اكتب سؤالك مباشرة ✍️"
        )

    elif "شخص" in text or "إنسان" in text:
        reply = (
            "👨‍💼 تم تسجيل طلبك\n"
            "سيتم تحويل رسالتك لأحد المسؤولين 👌\n"
            "يرجى الانتظار قليلًا ⏳"
        )

    elif any(word in text for word in ["شكرا", "شكراً", "merci", "thanks"]):
        reply = "العفو 😊🌷 يسعدنا خدمتك في أي وقت"

    elif any(word in text for word in ["باي", "وداعا", "إلى اللقاء"]):
        reply = "في أمان الله 👋🙂 نتشرف بزيارتك مرة أخرى"

    else:
        reply = (
            "وصلتني رسالتك 👍\n"
            "نراجعها حاليًا وسنرد عليك في أقرب وقت ⏳\n"
            "شكراً على ثقتك بنا 🤝"
        )

    # إرسال الرد للمستخدم
    await update.message.reply_text(reply)

# ===== تشغيل البوت =====
app = ApplicationBuilder().token(TOKEN).build()

app.add_handler(CommandHandler("start", start))
app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, auto_reply))

print("🤖 البوت يعمل الآن...")
app.run_polling()
